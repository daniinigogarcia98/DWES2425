...
// Establecemos la fecha actual en formato yyyy-mm-dd (es el formato que espera el input de tipo "date")
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Validación del checkbox +18 cuando se selecciona Terror
    ...
    if (empty($errors)) {
        // Convertir el array de géneros en un string separado por "/"
...
<body>
    ...
    <form action="" method="POST">
        ...
        <!-- Fecha de Registro (se inicializa con la fecha actual, pero el usuario puede elegirla) -->
        ...
        <select name="genres[]" id="genres" multiple size="5">
            <option value="Acción" <?= in_array("Acción", $genres) ? "selected" : "" ?>>Acción</option>
            <option value="Aventura" <?= in_array("Aventura", $genres) ? "selected" : "" ?>>Aventura</option>
            <option value="Comedia" <?= in_array("Comedia", $genres) ? "selected" : "" ?>>Comedia</option>
            <option value="Drama" <?= in_array("Drama", $genres) ? "selected" : "" ?>>Drama</option>
            <option value="Terror" <?= in_array("Terror", $genres) ? "selected" : "" ?>>Terror</option>
        </select><br><br>
